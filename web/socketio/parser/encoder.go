...
func (e *Encoder) Encode(h Header, args ...interface{}) (err error) {
	...
	if err != nil {
		logrus.Error("next writer session text:", err)
	...
	if err != nil {
		logrus.Error("write packet header with args:", err)
	...
	for _, b := range buffers {
		...
		if err != nil {
			logrus.Error("next writer session binary:", err)
		...
		if err != nil {
			logrus.Error("write packet buffer:", err)
...
func (e *Encoder) writeBuffer(w io.WriteCloser, buffer []byte) error {
	defer func() {
		if closeErr := w.Close(); closeErr != nil {
			logrus.Error("close writer:", closeErr)
		}
	}()
